...
lowest cost. Cost is `est.` in this run (chars÷4) because the scorecard was
generated by the pre-propagation binary; token-propagation has landed (the
provider now parses `usage`), so the next run reports exact cost.
...
1. **Re-run the matrix to report exact cost.** Token-propagation has landed (the
   provider parses `usage`, threads counts through to `estimate_cost`), but the
   current scorecard's cost column is still chars÷4 because that run used the
   pre-propagation binary. The next run reports real cost.
...
- **Cost** in the *current* scorecard is labeled `est.` because that run used
  the pre-propagation binary (chars ÷ 4). Token-propagation has landed — the
  Kotlin provider now parses `usage`, and counts flow through
  `CompletionResult → SkillResult → InvokeResponse` (HTTP) / a `_meta` JSON line
  (subprocess) into `estimate_cost`, which uses real tokens when present. The
  next matrix run reports exact cost. **Latency is already real** (wall-clock in
  the Python adapter).
