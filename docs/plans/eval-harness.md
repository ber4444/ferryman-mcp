# Plan: Eval Harness for One OpenClaw Skill

Target: the OpenClaw repository (not the chess-app repo). Suggested location for this file:
`docs/plans/eval-harness.md` inside that repo.

## Context for the agent

OpenClaw is a local-first MCP orchestration layer above Claude Code — pluggable skills,
...
work that makes OpenClaw itself presentable enough to publish — a repo with an eval harness and
...
- **Python for the harness**, deliberately — even if OpenClaw's core is TypeScript/Kotlin/other.
...
- **Don't touch OpenClaw's production routing or skill code** beyond what's needed to invoke a
...
- Identify the most-invoked skill from OpenClaw's own logs, or default to the company/role
...
- `eval_harness/invoke.py` — a thin adapter that calls the target OpenClaw skill programmatically
...
- Read OpenClaw's provider-routing config to enumerate the providers/models it can route the
  target skill to.
...
fabricated deltas. Publishing OpenClaw itself (making the repo public, if it isn't already)
